package ro.kudostech.authorizationserver.config;
...
public class SecurityConfig {
  ...
  public UserDetailsService userDetailsService() {
    var user1 = User.withUsername("user").password("password").authorities("read", "test").build();
  ...
  public OAuth2TokenCustomizer<JwtEncodingContext> tokenCustomizer() {
    return context -> {
      ...
      if (context.getTokenType().getValue().equals("access_token")) {
        context.getClaims().claim("authorities", authorities).claim("user", user.username());
      }
    };
  }
